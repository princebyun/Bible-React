    ...
    return (
        <div className="home-page">
            <div className="top-bar">
                <Link to="/" className="logo">Oikos Church Bible</Link>
            </div>
            <div className="main-content">
                <div className="search-wrapper">
                    ...
                    <form onSubmit={handleSearch} className="search-form">
                        ...
                        <input ref={keywordRef} className="search-input" type="search" name="keyword"
                               placeholder="성경 구절이나 단어를 검색해보세요..." aria-label="Search"/>
                    </form>
                </div>
                <div className="slider-container">
                    <button type="button" className="slider-btn prev" aria-label="이전"
                            style={{opacity: prevVisible ? 1 : 0, visibility: prevVisible ? 'visible' : 'hidden'}}
                            onClick={() => {
                                const w = cardWrapperRef.current;
                                if (w) w.scrollLeft -= getScrollAmount();
                            }}>&lt;</button>
                    ...
                    <button type="button" className="slider-btn next" aria-label="다음"
                            style={{opacity: nextVisible ? 1 : 0, visibility: nextVisible ? 'visible' : 'hidden'}}
                            onClick={() => {
                                const w = cardWrapperRef.current;
                                if (w) w.scrollLeft += getScrollAmount();
                            }}>&gt;</button>
